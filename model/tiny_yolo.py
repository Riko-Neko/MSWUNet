...
def decode_F_yolo(preds, S, B, conf_thresh=0.25, iou_threshold=0.5):
    """
    ...
    This function returns list per image of (N, classes, f_starts_norm, f_stops_norm)
    Where:
      - N: number of detections after NMS
      - classes: list[int] class IDs
      - f_starts_norm, f_stops_norm: list[float] normalized in [0, 1] corresponding to frequency range
    All values sorted by f_start ascending.
    Only includes boxes that approximately span full x (time) dimension.
    """
    batch = preds.shape[0]
    device = preds.device
    out = []
    for i in range(batch):
        ...
        # Apply NMS
        kept_boxes = non_max_suppression(boxes, iou_threshold=iou_threshold)
        # Filter for full-span in x (time) and collect normalized freq info
        detections = []
        for box in kept_boxes:
            x1, y1, x2, y2, score, cls = box
            x_center = (x1 + x2) / 2
            w = x2 - x1
            if abs(x_center - 0.5) < 0.1 and w > 0.8:  # approximate full span in time
                # Convert to normalized [0,1] frequency coordinates
                f_start = y1
                f_stop = y2
                detections.append((f_start, f_stop, cls))
        N = len(detections)
        classes = [d[2] for d in detections]
        f_starts = [d[0] for d in detections]
        f_stops = [d[1] for d in detections]
        out.append((N, classes, f_starts, f_stops))
    return out
...
class TinyYOLO(nn.Module):
    ...
    def __init__(self, num_classes=2, S=32, B=3, base_channels=32):
        super().__init__()
        self.S = S
        self.B = B
        self.C = num_classes
...
class YOLOv1Loss(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        obj_loss = F.mse_loss(pred_conf * obj_mask_f.expand_as(pred_conf),
                              conf_target_exp * obj_mask_f.expand_as(pred_conf), reduction='sum')

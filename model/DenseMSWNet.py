...
class DenseBlock(nn.Module):
    """
    A dense fusion block with skip connections.
    """

    def __init__(self, in_chans_list, out_chans, bn_chans=None):
        super().__init__()
        total_ch = sum(in_chans_list)
        bn_chans = bn_chans or total_ch // 2

        self.reduce = nn.Sequential(
            nn.Conv2d(total_ch, bn_chans, kernel_size=1, bias=False),
            nn.BatchNorm2d(bn_chans),
            nn.ReLU(inplace=True),
        )
        self.fuse = nn.Sequential(
            nn.Conv2d(bn_chans, out_chans, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(out_chans),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self, features):
        """
        features: list of tensors from different encoder stages,
                  each shape [B, Ci, H, W] (注意空间尺寸对齐！)
        """
...
class ClsHead(nn.Module):
    def __init__(self, in_chans=1, hidden_dim=64, base_chans=32, dropout=0.1):
        super().__init__()

        self.conv1 = nn.Conv2d(in_chans, base_chans * 2, kernel_size=3, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(base_chans * 2)
        self.act1 = nn.ReLU(inplace=True)

        self.conv2 = nn.Conv2d(base_chans * 2, base_chans * 2, kernel_size=5, padding=2, bias=False)
        self.bn2 = nn.BatchNorm2d(base_chans * 2)
        self.act2 = nn.ReLU(inplace=True)

        self.conv3 = nn.Conv2d(base_chans * 2, base_chans * 2, kernel_size=3, padding=2, dilation=2, bias=False)
        self.bn3 = nn.BatchNorm2d(base_chans * 2)
        self.act3 = nn.ReLU(inplace=True)

        self.se = SEBlock(base_chans * 2, reduction=8)

        self.pool = nn.AdaptiveAvgPool2d(2)  # 保留 2x2 空间结构

        self.fc = nn.Sequential(
            nn.Linear(base_chans * 2 * 2 * 2, hidden_dim),
            nn.ReLU(inplace=True),
            nn.Dropout(dropout),
    ...
    def forward(self, x):
        feat1 = self.act1(self.bn1(self.conv1(x)))
        feat2 = self.act2(self.bn2(self.conv2(feat1)))
        feat3 = self.act3(self.bn3(self.conv3(feat2)))
...
class MSWNet(nn.Module):
    def __init__(self, in_chans=1, dim=64, levels=[2, 4, 8, 16], wavelet_name='db4'):
        ...
        # RFI分支 (稠密解码)
        self.db4 = DenseBlock([filters[3]] * 4, filters[3])
        self.db3 = DenseBlock([filters[2]] * 4, filters[2])
        self.db2 = DenseBlock([filters[1]] * 4, filters[1])
        self.db1 = DenseBlock([filters[0]] * 4, filters[0])

        self.ma_dec4_c = UpConvBlock(filters[4], filters[3])
        self.ma_dec3_c = UpConvBlock(filters[3], filters[2])
        self.ma_dec2_c = UpConvBlock(filters[2], filters[1])
        self.ma_dec1_c = UpConvBlock(filters[1], filters[0])

        self.ma_dec4_u = UpConvBlock(filters[3], filters[2], up=True)
        self.ma_dec3_u = UpConvBlock(filters[2], filters[1], up=True)
        self.ma_dec2_u = UpConvBlock(filters[1], filters[0], up=True)
        self.ma_dec1_u = UpConvBlock(filters[0], filters[0], up=True)

        self.mask_out = nn.Conv2d(filters[0], 1, kernel_size=1)

        # Criteria Head
        self.detection_head = ClsHead(in_chans)

    def _split_H(self, h):
        """
        Splits a list of wavelet bandpass coefficients into individual subbands.

        Args:
            h (list): List of J tensors, each with shape (N, C, 3, H, W).
        Returns:
            Tuple of 3*J tensors, ordered as:
            HH1, HL1, LH1, HH2, HL2, LH2, ..., HH_J, HL_J, LH_J
        """
        assert isinstance(h, list), "Expected H to be a list."
        assert all(idx.ndim == 5 and idx.shape[2] == 3 for idx in h), "Subbands must have shape (N, C, 3, H, W)."
        return tuple([idx[:, :, i, :, :] for idx in h for i in [2, 1, 0]])
    ...
    def forward(self, x):
        ...
        plogits = self.detection_head(deno)  # (B)
        # print(f" --- Signal Existence Probability: {plogits.shape} --- ")

        # RFI Mask Branch Decoder (With Skip Connections)
        assert bn.shape == lls[-1].shape, f"features must match ll_-1. ({bn.shape} != {lls[-1].shape})"
        ma_d4 = torch.cat([bn, lls[-1]], dim=1)  # (B, 16D, H//16, W//16)
        ma_d4 = self.ma_dec4_c(ma_d4)  # (B, 8D, H//16, W//16)
        ma_d4 = self.db4([ma_d4, *self._split_H(Hs[-1])])  # (B, 8D, H//16, W//16)
        ma_d4 = self.ma_dec4_u(ma_d4)  # (B, 4D, H//8, W//8)

        assert ma_d4.shape == lls[-2].shape, f"features must match ll_-2. ({ma_d4.shape} != {lls[-2].shape})"
        ma_d3 = torch.cat([ma_d4, lls[-2]], dim=1)  # (B, 8D, H//8, W//8)
        ma_d3 = self.ma_dec3_c(ma_d3)  # (B, 4D, H//8, W//8)
        ma_d3 = self.db3([ma_d3, *self._split_H(Hs[-2])])  # (B, 4D, H//8, W//8)
        ma_d3 = self.ma_dec3_u(ma_d3)  # (B, 2D, H//4, W//4)

        assert ma_d3.shape == lls[-3].shape, f"features must match ll_-3. ({ma_d3.shape} != {lls[-3].shape})"
        ma_d2 = torch.cat([ma_d3, lls[-3]], dim=1)  # (B, 4D, H//4, W//4)
        ma_d2 = self.ma_dec2_c(ma_d2)  # (B, 2D, H//4, W//4)
        ma_d2 = self.db2([ma_d2, *self._split_H(Hs[-3])])  # (B, 2D, H//4, W//4)
        ma_d2 = self.ma_dec2_u(ma_d2)  # (B, D, H//2, W//2)

        assert ma_d2.shape == lls[-4].shape, f"features must match ll_-4. ({ma_d2.shape} != {lls[-4].shape})"
        ma_d1 = torch.cat([ma_d2, lls[-4]], dim=1)  # (B, 2D, H//2, W//2)
        ma_d1 = self.ma_dec1_c(ma_d1)  # (B, D, H//2, W//2)
        ma_d1 = self.db1([ma_d1, *self._split_H(Hs[-4])])  # (B, D, H//2, W//2)
        ma_ou = self.ma_dec1_u(ma_d1)  # (B, D, H, W)

        mask = torch.sigmoid(self._re_TF(self.mask_out(ma_ou), orig_t, orig_f))  # (B, 1, H, W)
        # print(f" --- Mask shape: {mask.shape} --- ")

        return deno, mask, plogits
...
if __name__ == '__main__':
    ...
    denoised, rfi_mask, plogits = model(test_input)
    print(f"Input shape: {test_input.shape}")
    print(f"Denoised shape: {denoised.shape}")
    print(f"RFI Mask shape: {rfi_mask.shape}")
    print(f"Signal Probs: {plogits}")

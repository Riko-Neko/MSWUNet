...
def train_model(model, train_dataloader, valid_dataloader, criterion, optimizer, scheduler, device, mode='detection',
                ...
                log_interval=50, start_epoch=0, best_valid_loss=float('inf'), epoch_log=[]):
        ...
        for step in range(steps_per_epoch):
            ...
            if step % log_interval == 0:
                with open(step_log_file, 'a') as f:
                    if mode == 'detection':
                        f.write(
                            f"{epoch},{step},{total_loss.item():.6f},{detection_loss:.6f},{denoise_loss:.6f},{det_loss_loc:.6f},{det_loss_class:.6f},{det_loss_conf:.6f},{det_n_matched},{lambda_denoise.item() if hasattr(lambda_denoise, 'item') else lambda_denoise:.6f}\n")
                    else:  # "mask" as default
                        f.write(
                            f"{epoch},{criterion.step},{total_loss.item():.6f},{spectrum_loss.item():.6f},{ssim_loss.item():.6f},{rfi_loss.item():.6f},{pdet_loss.item():.6f},{current_alpha:.4f},{current_beta:.4f},{current_gamma:.4f},{current_delta:.4f}\n")
        ...
        if valid_dataloader and (epoch + 1) % valid_interval == 0:
            ...
            if valid_loss is not None and valid_loss < best_valid_loss:
                if valid_loss > 0.:
                    ...
                    best_epoch = epoch + 1
                    best_model_path = best_weights_file
                    save_dict = {
                        'epoch': epoch,

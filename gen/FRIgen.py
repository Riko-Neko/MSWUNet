...
def add_rfi(clean_spec, rfi_params, noise_std=0.0):
    ...
    Parameters:
    clean_spec : 2D array, shape (time_frames, n_chan)
    ...
    Returns:
    noisy_spec : 2D array, shape (time_frames, n_chan)
    ...
    """
    time_frames, n_chan = clean_spec.shape
    noisy_spec = clean_spec.copy()
    rfi_mask = np.zeros_like(clean_spec, dtype=bool)

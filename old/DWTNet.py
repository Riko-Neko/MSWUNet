...
class DWTNet(nn.Module):
    """
    Modified DWTNet with a encoder-decoder architecture using DWT/IWT.
    """

    def __init__(self, wavelet_name='db4', levels=3, base_channels=16,
                 subband_enhance_type='dense', att_type='WLAM'):
        super(DWTNet, self).__init__()
        self.levels = levels
        self.base_channels = base_channels
        # Define one-level DWT and IWT modules for each layer
        ...
        # Encoder convolutions: refine LL coefficients (1 channel -> base_channels -> 1 channel)
        ...
        # Decoder convolutions: refine IWT output (1 channel -> base_channels -> 1 channel)
        ...
        # Attention modules at decoder (e.g., WLAM) applied after each decode stage
        self.att_dec = nn.ModuleList([
            AttentionModule(channels=1, att_type=att_type) for _ in range(levels)
        ])
        # Output RFI mask convolution (unchanged)
        self.rfi_mask_conv = nn.Conv2d(1, 1, kernel_size=1)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        # Encoder: apply DWT at each level, store low/high-frequency coefficients
        sizes = []
        highs = []
        current = x
        for i in range(self.levels):
            # Store current spatial size for later IWT
            sizes.append((current.size(2), current.size(3)))
            # Perform one-level DWT (returns [LL, (LH, HL, HH)])
            coeffs = self.dwt_layers[i](current)
            LL, (LH, HL, HH) = coeffs[0], coeffs[1]
            highs.append((LH, HL, HH))
            # Refine LL with a small conv block
            current = self.enc_convs[i](LL)
        # Decoder: upsample via IWT and fuse with skip connections
        for i in range(self.levels - 1, -1, -1):
            LH, HL, HH = highs[i]
            # Inverse DWT to upsample, restoring spatial resolution to sizes[i]
            current = self.idwt_layers[i]([current, (LH, HL, HH)], sizes[i])
            # Refine and apply attention after upsampling
            current = self.dec_convs[i](current)
            current = self.att_dec[i](current)
        # Final reconstructed image and RFI mask
        reconstructed = current
        mask = self.sigmoid(self.rfi_mask_conv(current))

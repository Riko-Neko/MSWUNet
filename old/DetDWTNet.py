...
class FreqDetectionHead(nn.Module):
    """
    1D detection head producing (B, P, 3, T, F) raw outputs.
    Args:
      in_ch: number of input channels
      hidden_ch: number of hidden channels (default: half of in_ch)
      P: number of preds per cell
    """

    def __init__(self, in_ch: int, hidden_ch: Optional[int] = None, P: int = 2):
        super().__init__()
        hidden = hidden_ch or max(in_ch // 2, 128)
        self.conv1 = nn.Conv2d(in_ch, hidden, kernel_size=(1, 3), padding=(0, 1))
        self.conv2 = nn.Conv2d(hidden, hidden, kernel_size=(1, 3), padding=(0, 1))
        self.pred = nn.Conv2d(hidden, P * 3, kernel_size=1)
        self.P = P
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.pred(x)  # (B, P*3, T, F)
        B, C_out, T, Ff = out.shape
        out = out.view(B, self.P, 3, T, Ff).contiguous()
        return out


class FreqFPNDetector(nn.Module):
    """
    Frequency-only FPN detector.

    Args:
      in_channels_list: list of ints matching feat_list order (shallow->deep)
      out_channels: number channels after lateral conv
      P: number of preds per cell
      use_spp: whether to use SPP on deepest layer
      use_pan: whether to use PAN (bottom-up) path aggregation
    """

    def __init__(self, in_channels_list: List[int], out_channels: int = 256, P: int = 2,
                 head_hidden: Optional[int] = None, use_spp: bool = True, use_pan: bool = False):
        super().__init__()
        assert len(in_channels_list) >= 1
        ...
        self.laterals = nn.ModuleList([nn.Conv2d(c, out_channels, kernel_size=1) for c in in_channels_list])

        # post-fuse conv blocks
        self.fuse_convs = nn.ModuleList(
            [ConvBlock1D(out_channels * 2, out_channels) for _ in range(self.num_levels)])

        # optional SPP on deepest feature
        if self.use_spp:
            self.spp = SPP1D(in_ch=in_channels_list[-1])
        else:
            self.spp = None

        # heads for each level
        self.heads = nn.ModuleList(
            [FreqDetectionHead(out_channels, hidden_ch=head_hidden, P=self.P) for _ in range(self.num_levels)])

    def forward(self, feat_list: List[torch.Tensor]) -> List[torch.Tensor]:
        """
        feat_list: list of tensors [(B, C3, T3, F3), (B, C4, T4, F4), ...] shallow->deep
        returns: list of raw_preds from each head: [(B,P,3,T3,F3'), ...] aligned to chosen
                 pyramid levels (same ordering as feat_list)
        """
        assert len(feat_list) == len(self.in_channels_list)
        # verify time dims may differ; we will align per-fusion when needed
        Ts = [f.shape[2] for f in feat_list]
        ...
        for i, f in enumerate(feat_list):
            # convert (B,C,T,F) -> conv2d expects (B,C,T,F) so ok
            x = self.laterals[i](f)
            laterals.append(x)

        # optional SPP on deepest original feature
        if self.spp is not None:
            # apply SPP to the deepest original (before lateral conv) to enrich it
            deepest = feat_list[-1]
            deepest = self.spp(deepest)
            # re-run lateral conv on the enriched deepest
            laterals[-1] = self.laterals[-1](deepest)

        # top-down fusion (shallow->deep ordering provided, so process reversed)
        fused = [None] * self.num_levels
        # start from deepest
        fused[self.num_levels - 1] = laterals[self.num_levels - 1]

        for i in range(self.num_levels - 2, -1, -1):
            deeper = fused[i + 1]
            lateral = laterals[i]

            # align time between lateral and deeper. Prefer downsampling lateral to deeper
            # to avoid upsampling deep semantic features which can introduce artifacts.
            T_deep = deeper.shape[2]
            T_lat = lateral.shape[2]
            if T_lat != T_deep:
                lateral_al = align_time(lateral, T_tgt=T_deep, mode='auto')
            else:
                lateral_al = lateral

            # upsample frequency axis of deeper to match lateral_al's freq
            target_F = lateral_al.shape[-1]
            deeper_up = upsample_F(deeper, F_tgt=target_F)

            # now deeper_up and lateral_al have same (T, F) and can be concatenated
            cat = torch.cat([deeper_up, lateral_al], dim=1)
            fused[i] = self.fuse_convs[i](cat)

        # optional PAN: bottom-up path aggregation to enrich deeper layers with low-level detail
        if self.use_pan:
            # iterate shallow->deep and downsample
            for i in range(0, self.num_levels - 1):
                cur = fused[i]
                target_F = fused[i + 1].shape[-1]
                cur_down = downsample_F(cur, F_tgt=target_F)
                cat = torch.cat([cur_down, fused[i + 1]], dim=1)
                fused[i + 1] = self.fuse_convs[i + 1](cat)

        # heads: produce raw preds aligned to each fused level's freq grid
        out_preds = []
        for i in range(self.num_levels):
            out_preds.append(self.heads[i](fused[i]))
...
class DWTNet(nn.Module):
    def __init__(self, in_chans=1, dim=64, levels=[2, 4, 8, 16], wavelet_name='db4', extension_mode='reflect',
        ...
        # 1D Detection 分支
        # self.detector = FreqFPNDetector(in_channels_list=[filters[0], filters[0], filters[1], filters[2]],
        #                                 out_channels=256, P=P, head_hidden=128, use_spp=use_spp, use_pan=use_pan)

        self.detector = FreqFPNDetector(in_channels_list=[in_chans], out_channels=256, P=P, head_hidden=512,
                                        use_spp=use_spp, use_pan=use_pan)
    ...
    def forward(self, x):
        ...
        # feat_det = [ou, d2, d3, d4]  # [(B, D, H, W), (B, D, H//2, W//2), (B, 2D, H//4, W//4), (B, 4D, H//8, W//8)]
        # dets = self.detector(feat_det)  # List[(B, P, 3, T_i, F_i)]

        feat_det = [deno]
        dets = self.detector(feat_det)  # List[(B, P, 3, T_i, F_i)]
        # print(f" --- Detections shape: {[p.shape for p in dets]} --- ")
...
if __name__ == '__main__':
    ...
    if test_mode == 'det':
        ...
        B = 2
        # example per-level time sizes may differ
        T3 = 8
        T4 = 4
        T5 = 2
        # example per-level freq sizes: stride 4,8,16 => 256,128,64
        F3 = 256
        F4 = 128
        F5 = 64
        C3 = 128
        C4 = 256
        C5 = 512
        x3 = torch.randn((B, C3, T3, F3))
        x4 = torch.randn((B, C4, T4, F4))
        x5 = torch.randn((B, C5, T5, F5))
        feat_list = [x3, x4, x5]
        in_chs = [C3, C4, C5]
        block = FreqFPNDetector(in_channels_list=in_chs, out_channels=256, P=2, head_hidden=128, use_spp=True,
                                use_pan=True)
        preds = block(feat_list)
        for i, p in enumerate(preds):
            print(f"level {i}: pred shape = {p.shape}")
        # expected shapes: [(B,P,3,T3,F3), (B,P,3,T4,F4), (B,P,3,T5,F5)]
